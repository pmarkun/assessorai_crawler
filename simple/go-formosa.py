#!/usr/bin/env python3
"""
Simple Scraper for Legislative Proposals - Formosa GO

This script scrapes legislative proposals from the SAPL-based system of Câmara Municipal de Formosa, GO.
It processes the specified page of a proposals list, extracts metadata for each proposal, and prints the results.

Usage:
    python go-formosa.py --page 1 --tipo 18 --ano 2025

Arguments:
- --page: Page number to scrape (default: 1)
- --tipo: Document type ID (default: 18, e.g., 18 for Projeto de Lei Ordinária)
...
def extract_metadata_from_row(row, base_url):
    ...
    # Parse title to extract type, number, year (e.g., "PLOPL 105/2025")
    ...
    # Set fixed fields for Formosa
    item['house'] = 'Câmara Municipal de Formosa'
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description='Simple scraper for legislative proposals - Formosa GO')
    parser.add_argument('--page', type=int, default=1, help='Page number to scrape (default: 1)')
    parser.add_argument('--tipo', type=int, default=18, help='Document type ID (default: 18)')
