# assessorai_crawler/spiders/base_camarasempapel.py
...
class BaseCamarasempapelSpider(scrapy.Spider):
    """Classe base para spiders do sistema 'camarasempapel' (usado em várias câmaras)."""
    ...
    house = None
    domain = None
    default_tipo = 1  # Default tipo=1, pode ser sobrescrito
    extra_params = []  # Params extras na URL, ex.: ["procuraTexto=DocumentoInicial"]
    ...
    def __init__(self, ano=None, tipo=None, max_pages=None, *args, **kwargs):
        ...
        base_url = f"https://{self.domain}/spl/consulta-producao.aspx"
        params = [f"tipo={self.tipo}"]
        if self.ano:
